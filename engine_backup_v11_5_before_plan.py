...
def htf_trend_check(htf_candles):
    """v11.5: Uses ADX + RSI direction for HTF trend."""
    ...
    if rsi_now is None or rsi_prev is None:
        return None, adx, None
    if pdi > ndi and rsi_now >= rsi_prev:
        htf_dir = "call"
    elif ndi > pdi and rsi_now <= rsi_prev:
        htf_dir = "put"
    else:
        htf_dir = None
    return htf_dir, adx, f"PDI:{pdi:.1f}/NDI:{ndi:.1f}"
...
def decide(candles, name, entry_dur=60, htf_candles=None, adaptive=None):
    ...
    too_big = max_body > (avg_body * 3.5) if avg_body > 0 else False
    ...
    if adx >= dynamic_adx_min:
        ...
        _need_both = adaptive.get("require_both_support", False) if adaptive else False
        _min_support = 2 if _need_both else 1
        if _regime == "BREAKOUT":
            _min_support = 2

        # ── RANGING → Require Double Confirmation ──
        if _regime == "RANGING":
            _min_support = 2
...
async def get_eligible(client, tf_label, min_payout=85):
    try:
        ...
        for code,name in client.get_all_asset_name():
            if not any(p in name.upper() for p in FOREX_PAIRS): continue
...
async def fetch_htf_candles(client, code, htf_dur):
    for attempt in range(2):
        try:
            c = await asyncio.wait_for(
                client.get_candles(code, 0, HTF_CANDLES, htf_dur), timeout=10.0
            )
            if c and len(c) >= ADX_PERIOD * 2 + 2:
                if is_valid(c):
                    return c
        except:
            pass
...
class BotEngine:
    ...
    async def _fetch_and_analyze(self, eligible, entry_dur, tf_label, htf_dur):
        ...
        await self.emit("log", {
            "msg": f"[{now_str()}] SCANNING v11.5 — TF:{tf_label} HTF:{htf_label} (2-Candle Logic)"
        })
        ...
        async def fetch_one(name, code, payout):
            ...
            async with sem:
                ...
                if entry:
                    ...
                    if not cache or (now_ts - cache["time"]) >= (htf_dur * 0.9):
